...
require("lspconfig").bashls.setup({
    on_attach = function(client, bufnr)
        local msg = string.format("[lspconfig-sh] LSP client '%s' attached to buffer %d", client.name, bufnr)
...
local function format_with_beautysh(bufnr)
    if vim.fn.executable("beautysh") == 0 then
        vim.notify("[lspconfig-sh] beautysh command not found. Please install it (:MasonInstall beautysh)", vim.log.levels.WARN, { title = "Formatting" })
        return
    end
    ...
    vim.api.nvim_buf_call(bufnr, function()
        ...
        if vim.v.shell_error == 0 then
            -- 成功時
            vim.notify("[lspconfig-sh] Formatted buffer " .. bufnr .. " with beautysh", vim.log.levels.INFO, { title = "Formatting" })
        else
            -- 失敗時
            local msg = string.format("[lspconfig-sh] Failed to format with beautysh. Exit code: %d", vim.v.shell_error)
...
-- 一方、同様のフォーマット処理を実装しているlspconfig-lua.luaでは、ファイルを開いた直後からフォーマット処理を実行可能です。
